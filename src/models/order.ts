...
export class OrdersStore {
    ...
    async Create(o: Order): Promise<Order> {
        try {
            ...
            const result = await connect.query(sql, [o.productId, o.quantity, o.userId, o.status])
            connect.release()
            return result.rows[0]
        }catch (err) {
    ...
    async CurrentByUser(userId: number): Promise<Order> {
        try {
            ...
            throw new Error(`Could not find current order by user ${userId}. Error: ${err}`)
        }
    }
    ...
    async CompleteByUser(userId: number): Promise<Order[]> {
        try {
            ...
            throw new Error(`Could not find complete orders by user ${userId}. Error: ${err}`)
        }
    }
